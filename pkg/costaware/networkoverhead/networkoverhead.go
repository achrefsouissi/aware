...
var _ framework.PreFilterPlugin = &CustomNetworkOverhead{}
var _ framework.FilterPlugin = &CustomNetworkOverhead{}
var _ framework.ScorePlugin = &CustomNetworkOverhead{}

const (
	// Name : name of plugin used in the plugin registry and configurations.
	Name = "CustomNetworkOverhead"
	...
	// preFilterStateKey is the key in CycleState to CustomNetworkOverhead pre-computed data.
...
// CustomNetworkOverhead : Filter and Score nodes based on Pod's AppGroup requirements: MaxNetworkCosts requirements among Pods with dependencies
type CustomNetworkOverhead struct {
	client.Client
...
func (no *CustomNetworkOverhead) Name() string {
	return Name
}

func getArgs(obj runtime.Object) (*pluginconfig.CustomNetworkOverheadArgs, error) {
	CustomNetworkOverheadArgs, ok := obj.(*pluginconfig.CustomNetworkOverheadArgs)
	if !ok {
		return nil, fmt.Errorf("want args to be of type CustomNetworkOverhead, got %T", obj)
	}

	return CustomNetworkOverheadArgs, nil
}

// ScoreExtensions : an interface for Score extended functionality
func (no *CustomNetworkOverhead) ScoreExtensions() framework.ScoreExtensions {
	return no
}

// New : create an instance of a CustomNetworkOverhead plugin
func New(_ context.Context, obj runtime.Object, handle framework.Handle) (framework.Plugin, error) {
	klog.V(4).InfoS("Creating new instance of the CustomNetworkOverhead plugin")
...
func (no *CustomNetworkOverhead) PreFilter(ctx context.Context, state *framework.CycleState, pod *corev1.Pod) (*framework.PreFilterResult, *framework.Status) {
	...
	// Get AppGroup CR
	appGroup := no.findAppGroupCustomNetworkOverhead(agName)

	// Get NetworkTopology CR
	networkTopology := no.findNetworkTopologyCustomNetworkOverhead()
...
func (no *CustomNetworkOverhead) PreFilterExtensions() framework.PreFilterExtensions {
	return no
}

// AddPod from pre-computed data in cycleState.
// no current need for the CustomNetworkOverhead plugin
func (no *CustomNetworkOverhead) AddPod(ctx context.Context,
...
// no current need for the CustomNetworkOverhead plugin
func (no *CustomNetworkOverhead) RemovePod(ctx context.Context,
	cycleState *framework.CycleState,
	podToSchedule *corev1.Pod,
...
func (no *CustomNetworkOverhead) Filter(ctx context.Context,
	cycleState *framework.CycleState,
	pod *corev1.Pod,
	nodeInfo *framework.NodeInfo) *framework.Status {
...
func (no *CustomNetworkOverhead) Score(ctx context.Context,
	cycleState *framework.CycleState,
	pod *corev1.Pod,
...
func (no *CustomNetworkOverhead) NormalizeScore(ctx context.Context,
...
func (no *CustomNetworkOverhead) sortNetworkTopologyCosts(networkTopology *ntv1alpha1.NetworkTopology) {
...
func (no *CustomNetworkOverhead) populateCostMap(
...
func checkMaxNetworkCostRequirements(
	...
	zone string,
	costMap map[networkawareutil.CostKey]int64,
	no *CustomNetworkOverhead) (int64, int64, error) {
...
func (no *CustomNetworkOverhead) getAccumulatedCost(
	scheduledList networkawareutil.ScheduledList,
	dependencyList []agv1alpha1.DependenciesInfo,
	nodeName string,
	region string,
...
func getPreFilterState(cycleState *framework.CycleState) (*PreFilterState, error) {
	...
	if !ok {
		return nil, fmt.Errorf("%+v  convert to CustomNetworkOverhead.preFilterState error", no)
	}
	return state, nil
}

func (no *CustomNetworkOverhead) findAppGroupCustomNetworkOverhead(agName string) *agv1alpha1.AppGroup {
...
func (no *CustomNetworkOverhead) findNetworkTopologyCustomNetworkOverhead() *ntv1alpha1.NetworkTopology {
	klog.V(6).InfoS("namespaces: %s", no.namespaces)
	for _, namespace := range no.namespaces {
